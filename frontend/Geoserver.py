...
class Geoserver:
    ...
    def __init__(
        ...
        if os.path.exists(config_file_path):
            with open(config_file_path, "r") as config_file:
                ...
                self.username = config_data.get("GEOSERVER_USERNAME", "")
                self.service_url = config_data.get("GEOSERVER_SERVICE_URL", "")
                self.password = config_data.get("GEOSERVER_PASSWORD", "")
        else:
            raise FileNotFoundError("Config file 'config.json' not found")

        self.services = ServiceEnum
    ...
    def enable_service(self, workspace: str):
        """
        Enables the WMS, WFS, WMTS, and WCS services for a workspace.
        This function is executed right after a new workspace is created
        parameters:
        workspace: workspace name
        service: service name

        First, it gets the global serivce setting and then sets the service setting for workspace
        """
        # Enabling WFS service
        self._requests(
            "put",
            url=config.GLOBAL_SETTTINGS_URL.format(SERVICE="wfs", WORKSPACE=workspace, GEOSERVER_SERVICE_URL= self.service_url),
            data=config.WFS_GLOBAL_XML,
            headers={"Content-Type": "text/xml"},
        )

        # Enabling WMTS service
        self._requests(
            "put",
            url=config.GLOBAL_SETTTINGS_URL.format(SERVICE="wmts", WORKSPACE=workspace, GEOSERVER_SERVICE_URL= self.service_url),
            data=config.WMTS_GLOBAL_XML,
            headers={"Content-Type": "text/xml"},
        )

        # Enabling WMS service
        self._requests(
            "put",
            url=config.GLOBAL_SETTTINGS_URL.format(SERVICE="wms", WORKSPACE=workspace, GEOSERVER_SERVICE_URL= self.service_url),
            data=config.WMS_GLOBAL_XML,
            headers={"Content-Type": "text/xml"},
        )
    ...
    def create_S3Blobstore(
        self, name: str, prefix: str
    ):  # , bucket:str, accessKey:str, secretKey:str):
        s3_blobstore_xml = f"""<?xml version=\"1.0\" encoding=\"UTF-8\"?><S3BlobStore>
          <default>false</default> <id>{name}</id> <enabled>true</enabled> 
          <bucket>{config.S3_BUCKET}</bucket> <prefix>geoserver/{prefix}/</prefix> 
          <awsAccessKey>{config.AWS_ACCESS_KEY}</awsAccessKey> 
          <awsSecretKey>{config.AWS_SECRET_KEY}</awsSecretKey> 
          <access>PUBLIC</access> <maxConnections>50</maxConnections> 
        ...
        existing_blobstores = self._requests(
            "get",
            f"{self.service_url}/gwc/rest/blobstores",
            headers={"accept": "application/json", "content-type": "application/json"},
        ).json()

        print(existing_blobstores)
        # if name in existing_blobstores:
        #     raise GeoserverException(status="500", message="Blobstore already exists")

        r = self._requests(
            method="put",
            url=f"{self.service_url}/gwc/rest/blobstores/{name}",
            data=json.dumps(s3_blobstore_json),
    ...
    def seed_tiles(
        ...
        print(f"seed json: {seed_tiles_json}")
        seed_rest_url = f"{self.service_url}gwc/rest/seed/{layer_name}.json"
        headers = {"content-type": "application/json", "accept": "application/json"}
        r = self._requests(
            method="post",
            url=seed_rest_url,
            data=json.dumps(seed_tiles_json),
            headers=headers,
        )
        print(f"geoserver seed response: {r.status_code, r.content}")
    ...
    def import_Tif(self, workspace: str, link: str):
        import_url = f"{self.service_url}/rest/imports?async=true&exec=true"
        ...
        response = self._requests(
            method="post", url=import_url, json=import_payload, headers=headers
        )
        print(f"import {response.content}")
        # if response.status_code not in [200, 201]:
        #     raise Exception(f"Error creating import context: {response.content}")

        import_id = response.json().get("import", {}).get("id", None)
        print(f"{import_id=}")
        # if not import_id:
        #     raise Exception(f"Failed to get import ID: {response.content}")

        print(f"Import context created successfully with import_id={import_id}")
        return response.json()

    def get_import_state(self, import_id: int):
        state_url = f"{self.service_url}/rest/imports/{import_id}"

        st_response = self._requests(method="get", url=state_url)
        return st_response.json()["import"]["state"]
    ...
    def get_layers(self, workspace: Optional[str] = None):
        ...
        print(url)
        r = self._requests(method="get", url=url)
        if r.status_code == 200:
            layers = r.json()
            for layer in layers["layers"]["layer"]:

                # layer['href']=layer['href'].replace('https', 'http')
                print(layer)
                layerJson = self._requests(
                    method="get", url=str(layer["href"]), verify=False
                ).json()
                print(layerJson)
                layer["type"] = layerJson["layer"]["type"]
                # layer['date']=layerJson['layer']['dateCreated']
            return layers
    ...
    def check_layer_exists(self, workspace, layer_name):
        print(f'checking layer on geoserver: {layer_name}')
        """Checks if a layer exists on GeoServer and returns its type."""
        try:
            layers= self.get_layers(workspace)
            
            for layer in layers:
                if layer["name"].lower() == layer_name.lower():
                    return layer["name"], layer["type"].lower()
            return "Layer not found"
        except requests.exceptions.RequestException as e:
            return f"Error checking layer: {str(e)}"
    ...
    def enable_tile_cache(
        self, layer_name: str, workspace: Optional[str] = None, blobStoreId: str = None
    ):
        layer_url = config.GWC_REST_LAYER_URL.format(layer_name=layer_name, GEOSERVER_SERVICE_URL=self.service_url)
        caching_xml = config.TILE_CACHING_XML.format(
            layer_name=layer_name, blobStoreId=blobStoreId
        )
        headers = {"content-type": "text/xml"}

        r = self._requests("put", url=layer_url, data=caching_xml, headers=headers)
        if r.status_code == 201 or r.status_code == 200:
            return "{}Caching enabled for layer: {}".format(r.status_code, layer_name)
        else:
            raise GeoserverException(r.status_code, r.content)
    ...
    def create_shp_datastore(
        ...
        print(f" got path as {type(path)}: {path}")
        ...
        if isinstance(path, dict):
            print(f" got path as dict: {path}")
            path = prepare_zip_file(store_name, path)
        # print(f"{prepare_zip_file(store_name, path)}")
